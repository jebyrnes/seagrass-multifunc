...
<center>
```{r, echo=FALSE, fig.height=6, fig.width=11.2, fig.align='center'}
...
germanLines+
  ...
  annotate(geom="text", x=16.85, y=c(germanIDX$Mmin, germanIDX$Mmax, germanIDX$Mmde), label=c("   ", "    ", "    "))
```   
...
germanLines + 
  ...
  annotate(geom="text", x=16.85, y=c(germanIDX$Mmin, germanIDX$Mmax, germanIDX$Mmde), label=c("    ", "    ", "    "), size=7)
```
...
germanLines + 
  geom_smooth(method="glm", family=quasipoisson(link="identity"), 
              fill=NA, aes(color=percent), data=germanyThresh[which(germanyThresh$IDX>0),], lwd=8) +
  annotate(geom="text", x=0, y=c(0.25,2,4.7), label=c("Tmax", "Tmde", "Tmin"), size=8) +
  annotate(geom="text", x=16.85, y=c(germanIDX$Mmin, germanIDX$Mmax, germanIDX$Mmde), label=c("Mmin", "Mmax", "Mmde"), size=7)
```
